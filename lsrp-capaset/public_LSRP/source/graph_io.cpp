    ...
    std::string map_file_path, std::vector<std::vector<double> >* output)
{
    ...
    *output = grid;
	...
	int LoadNodeCapacities(std::string capacity_file, std::unordered_map<long, int>* node_capacities) {
		std::ifstream fin;
		fin.open(capacity_file);
		if (!fin) {
			std::cerr << "[Error] file '" << capacity_file << "' could not be opened" << std::endl;
			return -1;
		}
	
		long node_id;
		int capacity;
		while (fin >> node_id >> capacity) {
